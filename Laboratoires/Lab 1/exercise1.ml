...
let myfavourite:string = favourizer "Java";;
...
let odd (x: int) : bool = if even x then false else true;;
...
let why1 = (1.0 = 1.0)
let why2 = (1.0 == 1.0)
let because = "a single equal (=) compares objects by value. Whereas a double equals (==) compares objects by their address.\n
in why2, the first 1.0 is stored is an address that is different from the second 1.0.\n
StackOverflow: https://stackoverflow.com/a/13590362\n
Page 263 of Real World OCaml Textbook states that (==) checks for physical equality, whereas (=) checks of structural equality."
